	...
	return () => {
		...
		vite.middlewares.use(async (req, res) => {
			try {
				...
				if (user_hooks.getEnv) {
					const { set_runtime_env } = await vite.ssrLoadModule(
						process.env.BUNDLED
							? `/${posixify(
									path.relative(cwd, `${svelte_config.kit.outDir}/runtime/env.js`)
							  )}`
							: `/@fs${runtime}/env.js`
					);
					set_runtime_env(user_hooks.getEnv());
				}
